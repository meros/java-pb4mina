...
public class ProtoBufDecoder extends CumulativeProtocolDecoder {
	...
	private static Message readMessage(
		...
		if (remainingBytesInStream < packageLength) {
			...
			InputStream delimitedInputStream = new LengthDelimitedInputStream(inputStream.asInputStream(), packageLength);
	...
	private static int readLength(IoBuffer inputStream) throws IOException {
		...
		InputStream delimitedInputStream = new LengthDelimitedInputStream(inputStream.asInputStream(), packageLengthTokenSize);
